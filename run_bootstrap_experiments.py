...
def parse_args():
    parser = argparse.ArgumentParser(description="Bootstrap simulation runner")
    parser.add_argument(
        "--setting",
        type=str,
        required=True,
    ...
    parser.add_argument("--additional-transform-names", type=str, nargs="+", default=[])
...
def main():
    # Ensure relative paths in settings/bootstrap.py resolve even when invoked outside the repo root.
    repo_root = Path(__file__).resolve().parent
    os.chdir(repo_root)

    from settings.bootstrap import simulate_average
    ...
    cfg = DABConfig(
        ...
        additional_transform_names=args.additional_transform_names,
        label=args.label,
        seed=args.seed,
    ...
    simulate_average(cfg, args.setting)
